...
- `test_inspect_commit.py`—unit tests for the helper's parsing logic: pointer detection, header parsing per delimiter, the format a path names, the separator a revision's own bytes are written with (https://github.com/ponder-lab/Hugging-Face-Dataset-Mining/issues/79), and gzip decompression.
...
- `verified_by` is a handle rather than a full name, matching the form `gold_set.csv` already uses. The two annotators have no GitHub account, so their institutional identifiers stand in; an account minted to fill a CSV column would be worse than a handle that already identifies them.
